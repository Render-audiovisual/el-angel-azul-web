    ...
    try {
      ...
      await client.query(
        `update fichas_adhesion set
           ...
           acepta_condiciones = case when $22 = '' then acepta_condiciones else $22::boolean end,
           firma_storage_path = coalesce(nullif($23, ''), firma_storage_path),
           estado_revision = $24,
           documentacion_estado = $25,
           ficha_medica_estado = $26,
           autorizacion_estado = $27,
           observaciones = $28
         where id = $1`,
        [
          row.id,
          ...
          normalizeText(row.domicilio_provincia),
          normalizeText(row.domicilio_codigo_postal),
          normalizeText(row.acepta_condiciones),
          normalizeSignatureDataUrl(row.firma_data_url) || "",
          estadoRevision,
          normalizeText(row.documentacion_estado) || "pendiente",
          normalizeText(row.ficha_medica_estado) || "pendiente",
