...
async function findOrCreatePersona(queryable, { numeroDocumento, nombre, nacimiento, telefono }) {
  ...
  const nacimientoValido = ISO_DATE_RE.test(String(nacimiento || "")) ? nacimiento : null;
  ...
  const saveResult = await runBatch(rows, async (row) => {
    ...
    await pool.query(
      ...
      [
        ...
        Number(row.pasajeros_esperados) || 0,
...
async function saveContratosAdmin(rows, deleteIds = [], actorUsername) {
  ...
  const saveResult = await runBatch(rows, async (row) => {
    ...
    if (grupoLegacyId) {
      const grupoResult = await pool.query(`select id from grupos where legacy_id = $1`, [grupoLegacyId]);
    ...
    await pool.query(
      ...
      [
        ...
        normalizeText(row.fecha_creacion) || null,
        normalizeText(row.observaciones)
      ]
    );
  });
...
//
// persona + responsable se resuelven ANTES de abrir la transacción (mismo
// motivo que insertFichaPublica: no retener el lock de personas/responsables
// durante toda la transacción bajo carga concurrente) - pasajero +
// pasajero_responsables sí van juntos en una transacción real porque son
// la escritura correlacionada que pidió Wilson ("todo transaccional").
async function savePasajerosAdmin(rows, deleteIds = [], actorUsername) {
  ...
  const saveResult = await runBatch(rows, async (row) => {
    ...
    if (!dni) throw friendlyError("Falta el DNI del pasajero.");
    const grupoLegacyId = normalizeText(row.grupo_id);
    ...
    if (contratoLegacyId) {
      const contratoResult = await pool.query(`select id from contratos where legacy_id = $1`, [contratoLegacyId]);
    ...
    try {
      await client.query("BEGIN");
      const responsableId = await findOrCreateResponsable(client, {
...
function parseDate(value) {
  return ISO_DATE_RE.test(String(value || "")) ? value : null;
}
...
// Espeja safeJson/splitPipe de turismoRowToTrip en app.js - un valor
// corrupto (JSON roto, no un array) cae a lista vacía, nunca explota.
    ...
    try {
      ...
      const viajeResult = await client.query(
        ...
        [
          ...
          parseDate(row.fecha_salida),
          parseDate(row.fecha_regreso),
          parseBool(row.salida_garantizada),
          normalizeText(row.estado) || "borrador",
          parseBool(row.destacado),
          parseNumeric(row.orden) ?? 999
        ]
      );
      ...
      await client.query(
        ...
        [
          ...
          parseNumeric(row.precio_desde),
          parseNumeric(row.precio_valor),
          normalizeText(row.moneda) || "USD",
          parseNumeric(row.precio_base_doble),
          parseNumeric(row.suplemento_single),
          parseNumeric(row.precio_menor),
          ...
          JSON.stringify(safeJsonArray(row.itinerario)),
          JSON.stringify(safeJsonArray(row.fotos))
        ]
      );
      await client.query("COMMIT");
    } catch (error) {
