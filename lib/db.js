...
// Recupera únicamente envíos que nunca llegaron a una resolución final. Los
// estados "error" requieren revisión manual para no insistir indefinidamente
// sobre una dirección inválida. "sin_configurar" sí se retoma cuando el
// servicio arranca más adelante con Resend habilitado.
async function listFichaEmailsPendientes(limit = 25) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 25, 100));
  const result = await getPool().query(
    `select id::text as id
     from fichas_adhesion
     where email_estado in ('pendiente', 'sin_configurar')
     order by created_at
     limit $1`,
    [safeLimit]
  );
  ...
  const result = await queryable.query(
    `insert into responsables (nombre, apellido, tipo_documento, numero_documento, telefono, email, cuil_cuit)
     ...
     on conflict (tipo_documento, numero_documento) where numero_documento is not null
       do update set
         numero_documento = excluded.numero_documento,
         apellido = coalesce(responsables.apellido, nullif(excluded.apellido, '')),
